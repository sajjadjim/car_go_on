...
export default function MarketPriceGuide() {
  ...
  return (
    <section className="py-16 px-4 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 border-b border-slate-800">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight">
            Know the Fair Market Value <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-yellow-200">Before You Buy or Sell</span>
          </h2>
          ...
          {/* Toggle */}
          <div className="mt-6 inline-flex p-1 rounded-2xl bg-slate-950 border border-slate-800">
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.map((item, idx) => (
            <Link
              key={idx}
              href={item.link}
              className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 hover:border-slate-700 hover:bg-slate-900 transition-all duration-200 flex flex-col justify-between group"
            >
              <div>
                <div className="flex items-center justify-between text-xs text-slate-400 mb-1">
                  <span>{item.engine}</span>
                  <span className="text-slate-500 font-mono">BD Index</span>
                </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-end justify-between">
                <div>
                  <span className="text-[11px] text-slate-400 block">Est. Market Rate:</span>
                  <span className={`text-base font-extrabold ${activeTab === "cars" ? "text-amber-400" : "text-emerald-400"}`}>
                    {item.priceRange}
                  </span>
                </div>
                <span className="text-xs text-slate-400 group-hover:text-white transition flex items-center gap-1">
        ...
        <div className="mt-10 p-6 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-white">Need vehicle financing in Bangladesh?</h4>
